...
#include "Services/Exceptions/InvalidSku.hpp"
#include "Services/Loggers/FakeLogger.hpp"
#include "Services/MessageBus/MessageBus.hpp"
#include "Services/UoW/FakeUnitOfWork.hpp"
...
namespace Allocation::Tests
{
    TEST(Handlers, test_for_new_product)
    {
        ...
        Services::MessageBus::Instance().Handle(FakeUowFactory,
            std::make_shared<Domain::Commands::CreateBatch>("b1", "CRUNCHY-ARMCHAIR", 100));
    ...
    TEST(Handlers, test_for_existing_product)
    {
        ...
        messagebus.Handle(FakeUowFactory,
            std::make_shared<Domain::Commands::CreateBatch>("b1", "GARISH-RUG", 100));
        messagebus.Handle(FakeUowFactory,
            std::make_shared<Domain::Commands::CreateBatch>("b2", "GARISH-RUG", 99));
    ...
    TEST(Handlers, test_returns_allocation)
    {
        ...
        auto& messagebus = Services::MessageBus::Instance();
        messagebus.Handle(FakeUowFactory,
            std::make_shared<Domain::Commands::CreateBatch>("b1", "COMPLICATED-LAMP", 100));

        auto result = messagebus.Handle(FakeUowFactory,
            std::make_shared<Domain::Commands::Allocate>("o1", "COMPLICATED-LAMP", 10));
    ...
    TEST(Handlers, test_errors_for_invalid_sku)
    {
        ...
        Services::MessageBus::Instance().Handle(
            FakeUowFactory, std::make_shared<Domain::Commands::CreateBatch>("b1", "AREALSKU", 100));
    ...
    TEST(Handlers, test_commits)
    {
        ...
        auto& messagebus = Services::MessageBus::Instance();
        messagebus.Handle(FakeUowFactory,
            std::make_shared<Domain::Commands::CreateBatch>("b1", "OMINOUS-MIRROR", 100));
        messagebus.Handle(FakeUowFactory,
            std::make_shared<Domain::Commands::Allocate>("o1", "OMINOUS-MIRROR", 10));
    ...
    TEST(Handlers, test_sends_email_on_out_of_stock_error)
    {
        auto handler =
            [](std::shared_ptr<Domain::IUnitOfWork> uow,
                std::shared_ptr<Domain::Events::OutOfStock> event) -> std::optional<std::string>
        {
        ...
        messagebus.Handle(FakeUowFactory,
            std::make_shared<Domain::Commands::CreateBatch>("b1", "POPULAR-CURTAINS", 9));
        messagebus.Handle(FakeUowFactory,
            std::make_shared<Domain::Commands::Allocate>("o1", "POPULAR-CURTAINS", 10));
    }
    ...
    TEST(Handlers, test_changes_available_quantity)
    {
        ...
        messagebus.Handle(FakeUowFactory,
            std::make_shared<Domain::Commands::CreateBatch>("batch1", "ADORABLE-SETTEE", 100));
        auto batch = uow->GetProductRepository().Get("ADORABLE-SETTEE")->GetBatches().front();
        EXPECT_EQ(batch.GetAvailableQuantity(), 100);

        messagebus.Handle(
            FakeUowFactory, std::make_shared<Domain::Commands::ChangeBatchQuantity>("batch1", 50));
    ...
    TEST(Handlers, test_reallocates_if_necessary)
    {
        ...
        Services::MessageBus::Instance().Handle(
            FakeUowFactory, std::make_shared<Domain::Commands::ChangeBatchQuantity>("batch1", 25));
